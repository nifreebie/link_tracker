...
@Component
public class CreateTagCommandBuilder extends CommandBuilder implements BotMessages {

    private final TelegramBot telegramBot;

    protected CreateTagCommandBuilder(StateRepository stateRepository, TelegramBot telegramBot) {
    ...
    public Command build(Update update) {
        ...
        stateRepository.setState(update.message().chat().id(), UserState.AWAITING_TAG_NAME);
        telegramBot.execute(new SendMessage(update.message().chat().id(), ENTER_TAG_NAME));
        return null;
    }
}
